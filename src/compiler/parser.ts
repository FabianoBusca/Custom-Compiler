...
export class Parser {
    ...
    private readonly ast: Program = ASTFactory.createProgram(
        [],
        {line: 1, column: 1},
    ...
    private peek(ahead = 0): Tag {
        return this.tokens[this.index + ahead].tag !== undefined
            ? this.tokens[this.index + ahead].tag
            : ((): never => {
                throw new Error("Unexpected end of file");
            })();
    }
    ...
    private expect(expected: Tag, message?: string): Token {
        if (this.isEOF() || this.peek() !== expected) {
            this.throwExpectedError(
                message ? message : `Expected ${Parser.TAGS.get(expected)}`,
                this.index - 1,
            );
        }
        return this.advance();
    ...
    private parseStatement(): Statement {
        if (
            this.check(Tag.NUM, Tag.STR, Tag.BOOL, Tag.UNDERSCORE, Tag.THIS, Tag.ID)
        ) {
    ...
    private parseDeclaration(elements: Identifier[]): Statement {
        if (
            this.check(Tag.STR, Tag.NUM, Tag.BOOL) ||
            (this.check(Tag.ID) &&
                this.peek(1) === Tag.LSP &&
                this.peek(2) === Tag.RSP)
        ) {
        ...
        if (this.check(Tag.SELF_INC, Tag.SELF_DEC)) {
            return this.parseVariableDeclaration(
                this.createVariableAssignments(elements),
            );
        }
    ...
    private parseTypedDeclaration(elements: Identifier[]): Statement {
        ...
        if (this.check(Tag.ID)) {
            const assignments: (VariableAssignment | VariableDeclaration)[] =
                this.createVariableAssignments(elements);
        ...
        if (this.check(Tag.COMMA, Tag.ASSIGN)) {
            const types = elements.map((el) =>
                ASTFactory.createType(el.name, 0, el.start, el.end),
            );
            types.push(type);
    ...
    private parseIdentifierDeclaration(elements: Identifier[]): Statement {
        ...
        if (
            op.kind === "MemberFunctionCall" ||
            op.kind === "UnaryExpression" ||
            op.kind === "FunctionCall"
        ) {
            if (elements.length !== 0)
                this.throwError(
                    "Cannot call a function inside a declaration",
                    this.index - 3,
                );
            return op;
        }
        ...
        if (op.kind === "Identifier") {
            if (this.check(Tag.ID)) {
                const type = ASTFactory.createType(
                    (op as Identifier).name,
                    0,
                    op.start,
                    op.end,
                );
    ...
    private parseVariableDeclaration(
        ...
        const declarations = ASTFactory.createVariableOperations(
            operations,
            null,
            [],
            operations[0].start,
    ...
    private parseVariableOperations(): (
        | VariableDeclaration
        | VariableAssignment
        )[] {
        ...
        while (this.match(Tag.COMMA)) {
            ...
            if (this.check(Tag.STR, Tag.NUM, Tag.BOOL))
                ...
                const id = this.advance();
                declaration = ASTFactory.createVariableAssignment(
                    ASTFactory.createIdentifier(id),
                );
            } else if (this.check(Tag.ID)) {
                if (this.peek(1) === Tag.LSP && this.peek(2) === Tag.RSP)
                    declaration = this.parseTypedVariableDeclaration();
                else {
                    const op = this.parsePostfixExpression(
                        ASTFactory.createIdentifier(this.advance()),
                    );
                    if (op.kind === "Identifier") {
                        if (this.check(Tag.ID)) {
                            const type = ASTFactory.createType(
                            ...
                            declaration = ASTFactory.createVariableDeclaration(
                                type,
                                identifier,
                            );
                        } else declaration = ASTFactory.createVariableAssignment(op);
                    } else if (
                        op.kind === "MemberAttribute" ||
                        op.kind === "ArrayElement"
                    ) {
    ...
    private parseUnderscoreDeclaration(elements: Identifier[]) {
        ...
        if (this.check(Tag.ASSIGN)) {
            if (elements.length !== 1)
                this.throwError("Void function cannot have multiple return types");
    ...
    private parseAssignOperator(
        elements: Identifier[],
    ): VariableOperations | FunctionDeclaration {
        if (!(this.peek(1) === Tag.ID || this.peek(1) === Tag.UNDERSCORE))
            return this.parseVariableDeclaration(
                this.createVariableAssignments(elements),
            );
    ...
    private isFunctionDeclaration(): boolean {
        return (
            this.peek(1) === Tag.LRP &&
            ((this.peek(2) === Tag.RRP && this.peek(3) === Tag.LCP) ||
                ([Tag.STR, Tag.NUM, Tag.BOOL] as Tag[]).includes(this.peek(2)) ||
                (this.peek(2) === Tag.ID &&
                    (this.peek(3) === Tag.ID ||
                        (this.peek(3) === Tag.LSP && this.peek(4) === Tag.RSP))))
        );
    }
        ...
        while (true) {
            if (this.check(Tag.LSP)) {
                ...
                if (this.check(Tag.LRP)) {
                    ...
                    base = ASTFactory.createMemberFunctionCall(base, func);
                } else {
                    base = ASTFactory.createMemberAttribute(base, identifier);
                }
            } else if (this.check(Tag.INC, Tag.DEC)) {
                const operator = this.advance();
                base = ASTFactory.createUnaryExpression(
                    operator.tag,
                    base,
                    base.start,
                    operator.end,
    ...
    private parseFunctionCall(identifier: Identifier): FunctionCall {
        const call = ASTFactory.createFunctionCall(
            identifier,
            [],
            identifier.start,
            null as unknown as Location,
    ...
    private parseArrayElement(base: Expression): ArrayElement {
        const element = ASTFactory.createArrayElement(base, [], base.start, null as unknown as Location);
        this.advance(); // [
        do {
    ...
    private parseFString(): FString {
        const string: FString = ASTFactory.createFString(
            [],
            this.previous().start,
            null as unknown as Location,
        ); // '
    ...
    private parsePrintStatement(start: Location): PrintStatement {
        const statement = ASTFactory.createPrintStatement([], start, null as unknown as Location);
        this.expect(Tag.LRP);
        if (!this.match(Tag.RRP)) {
            do {
    ...
    private parseReadStatement(start: Location): ReadStatement {
        ...
        const statement = ASTFactory.createReadStatement([], start, null as unknown as Location);
        this.match(Tag.LRP);
        do {
            const expr = this.parseExpression();
            if (expr.kind === "FunctionCall" || expr.kind === "MemberFunctionCall")
                this.throwError("Cannot call a function inside a read statement");
    ...
    private parseReturnStatement(start: Location): ReturnStatement {
        const statement = ASTFactory.createReturnStatement([], start, null as unknown as Location);
        this.match(Tag.LRP);
        if (!this.match(Tag.RRP)) {
            do {
    ...
    private parseIfStatement(): IfStatement {
        ...
        return ASTFactory.createIfStatement(
            this.parseCondition(),
            this.parseBlock(),
            this.match(Tag.ELSE) ? this.parseBlock() : [],
            start,
            this.previous().end,
        );
    }
    ...
    private parseSwitchStatement(): SwitchStatement {
        ...
        const statement = ASTFactory.createSwitchStatement(
            this.parseExpression(),
            [],
            [],
            start,
    ...
    private parseClassDeclaration(): ClassDeclaration {
        ...
        const identifier = ASTFactory.createIdentifier(id);
        const declaration: ClassDeclaration = ASTFactory.createClassDeclaration(
            identifier,
            [],
            start,
            null as unknown as Location,
